...
const SOCIALS: Record<string, SocialDef> = {
  ...
  'xyz.farcaster': { label: 'Farcaster', build: v => `https://farcaster.xyz/${v}` },
  'app.bsky': { label: 'Bluesky', build: v => `https://bsky.app/profile/${v}` },
  'xyz.lens': { label: 'Lens', build: v => `https://hey.xyz/u/${v}` },
  'com.reddit': { label: 'Reddit', build: v => `https://reddit.com/user/${v}` },
  // Record value per ENSIP-18 is the username; a numeric user ID or a full
  // URL (e.g. a discord.gg invite) also works.
  'com.discord': { label: 'Discord', build: v => `https://discord.com/users/${v}` },
  'com.linkedin': { label: 'LinkedIn', build: v => `https://www.linkedin.com/in/${v}` },
  'com.facebook': { label: 'Facebook', build: v => `https://www.facebook.com/${v}` },
